...
void CreateWonderTradePokemon(u8 whichPlayerMon)
{
	...
	if (currHeldItem == ITEM_NONE)
	{
		for (i = 0; i < EVOS_PER_MON; i++)
		{
			...
			{
				// 30% chance for the in coming Pok�mon to hold an Everstone if they evolve by trading
				if (Random() % 255 <= 77)
				{
			...
			{
				// 30% chance for the in coming Pok�mon to hold the item they need to evolve if they need one
				if (Random() % 255 <= 77)
				{
...
#ifdef POKEMON_EXPANSION
	// 10% chance of giving the in coming Pok�mon their HA, if they have one
...
// Generate an item randomly for a Wonder Trade in coming Pok�mon to hold, with a few exceptions
u16 GetValidWonderTradeItem(u16 item)
{
...
ROLL:
	...
	// Make sure that if the Pok�mon can Mega Evolve, or it evolves into a species who can, it can get the relevant Mega Stone
